...
AForm::AForm(): name("A AForm"), sign_grade(1),exe_grade(1), _signed(0){
...
int AForm::checkExeGrade(const int exe_grade) const{
    if (exe_grade < 1)
...
int AForm::checkSignGrade(const int sign_grade) const{
    if (sign_grade < 1)
        throw (GradeTooHighException());
