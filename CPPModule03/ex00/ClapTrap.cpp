...
ClapTrap::ClapTrap( std::string ct_name ): name(ct_name), HitPoints(10), EnergyPoints(10), AttackDamage(0){
	std::cout << "A Claptrap has been borned and it's name is " << name << std::endl;
}
...
void ClapTrap::takeDamage( unsigned int amount )
{
	...
	if ( this->HitPoints < 0 )
		this->HitPoints = 0;
	std::cout << "ClapTrap " << this->name << " receives " << amount << " points of damage and have "  << this->HitPoints << " life left" << std::endl;
	if ( HitPoints <= 0 )
	{
		std::cout << "ClapTrap " << this->name << " won't clap anymore, 'cause he dead now" << std::endl;
		return ;
	}
}
...
void ClapTrap::beRepaired( unsigned int amount ){
	...
	{
		std::cout << "ClapTrap " << this->name << " can't repair itself 'cause he's out of energypoints" << std::endl;
		return ;
	}
	this->EnergyPoints -= 1;
	this->HitPoints += amount;
	if ( this->HitPoints > 10 )
		this->HitPoints = 10;
	std::cout << "ClapTrap " << this->name << " repairs itself of " << amount << " points of damage, have " << this->HitPoints << " life points left and " << this->EnergyPoints << " energy points left" << std::endl;
}
...
std::ostream &operator << ( std::ostream& out, ClapTrap & claptrap){
	out << "Name: " << claptrap.get_name() << ", Attack: " << claptrap.get_attack() << std::endl;
	return out;
}
